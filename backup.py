...
def get_version():
    try:
        with open('app/build.gradle', 'r') as f:
            content = f.read()
            match = re.search(r'versionName\s+"v?([\d\.]+)', content)
            if match:
...
def backup():
    ...
    # Organized folder: ../backup/openlauncher-maya
    backup_root = os.path.abspath(os.path.join(source_dir, '..', 'backup', 'openlauncher-maya'))
    
    if not os.path.exists(backup_root):
        os.makedirs(backup_root)

    # Find the latest backup of this version to compare against
    backups = [d for d in os.listdir(backup_root) if d.startswith(f'v{version}')]
    backups.sort(key=natural_sort_key)
    ...
    if not existing_backups:
        target_name = f'v{version}_full'
    else:
        # Simple suffix increment logic
        count = len(existing_backups)
        target_name = f'v{version}_inc_{count}'

    target_dir = os.path.join(backup_root, target_name)
    
    exclude = {'.git', 'build', '.gradle', '.idea', 'backup.py', 'local.properties', 'debug.keystore'}
    changed_files = []

    print(f"Backing up OpenLauncher v{version}...")
    if latest_backup:
    ...
    # Store the name for build.sh to link status
    try:
        with open("LAST_BACKUP_NAME", "w") as f:
            f.write(target_name)
    except:
        pass
